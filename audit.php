...
<head>
    ...
    <style>
        * { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        .table-wrap { background: white; border-radius: 12px; border: 1px solid #e8e8e8; overflow: hidden; overflow-x: auto; }
...
<div class="main">
    ...
    <div class="two-col">
            ...
            <form method="POST">
                <div class="form-group">
                    ...
                    <select name="medicine_id" required>
                        ...
                        <?php
                        $meds = mysqli_query($conn, "SELECT m.id, m.name, sl.current_quantity FROM medicines m JOIN stock_levels sl ON m.id = sl.medicine_id ORDER BY m.name ASC");
        ...
        <div class="card">
            ...
            <?php
            $recent = mysqli_query($conn, "SELECT al.*, m.name as medicine_name FROM audit_log al JOIN medicines m ON al.medicine_id = m.id ORDER BY al.checked_at DESC LIMIT 5");
    ...
    <div class="table-wrap">
        <table>
            <thead>
                <tr>
                    <th>#</th><th>Medicine</th><th>Recorded Qty</th><th>Physical Qty</th><th>Discrepancy</th><th>Audited By</th><th>Date</th><th>Status</th>
                </tr>
            </thead>
            <tbody>
            <?php
            ...
            if (mysqli_num_rows($audits) > 0) {
                ...
                while ($row = mysqli_fetch_assoc($audits)) {
                    ...
                    $date = date('d M Y', strtotime($row['checked_at']));
                    echo "<tr><td>{$i}</td><td>{$row['medicine_name']}</td><td>{$row['recorded_qty']}</td><td>{$row['physical_qty']}</td><td>{$disc}</td><td>{$row['user_name']}</td><td>{$date}</td><td>{$status}</td></tr>";
                    $i++;
                }
            } else {
