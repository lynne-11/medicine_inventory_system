...
if (isset($_POST['stock_in'])) {
    ...
    if ($quantity <= 0) {
        ...
        $new_qty = ($current['current_quantity'] ?? 0) + $quantity;
        mysqli_query($conn, "UPDATE stock_levels SET current_quantity = $new_qty WHERE medicine_id = $medicine_id");
        $sql = "INSERT INTO stock_transactions (medicine_id, user_id, type, quantity, notes) VALUES (?, ?, 'stock_in', ?, ?)";
...
<head>
    ...
    <title>PharmTrack - Record Stock In</title>
    <style>
...
<div class="sidebar" id="sidebar">
    ...
    <a href="storekeeper_stock_in.php" class="nav-item active">Record Stock In</a>
...
<div class="main">
    <div class="topbar">
        <div><h1>Record Stock In</h1><p>Receive medicines from suppliers</p></div>
    ...
    <div class="two-col">
        <div class="card">
            <h3>Receive Medicine from Supplier</h3>
            <form method="POST">
                ...
                <div class="form-group">
                    ...
                    <input type="number" name="quantity" placeholder="Enter quantity received" min="1" required>
                </div>
                <div class="form-group">
                    <label>Notes (optional)</label>
                    <input type="text" name="notes" placeholder="e.g. Received from KEMSA">
                </div>
                <button type="submit" name="stock_in" class="btn-primary">Receive Stock (Stock In)</button>
            </form>
        </div>
        <div class="card">
            ...
            <table>
                ...
                <tbody>
                <?php
                $meds2 = mysqli_query($conn, "SELECT m.*, sl.current_quantity FROM medicines m JOIN stock_levels sl ON m.id = sl.medicine_id ORDER BY sl.current_quantity ASC");
    ...
    <div class="section-title">My Stock In History</div>
    <div class="table-wrap">
        <table>
            <thead><tr><th>Medicine</th><th>Qty Received</th><th>Notes</th><th>Date</th><th>Time</th></tr></thead>
            <tbody>
            <?php
            $trans = mysqli_query($conn, "SELECT st.*, m.name as medicine_name FROM stock_transactions st JOIN medicines m ON st.medicine_id = m.id WHERE st.user_id = {$_SESSION['user_id']} AND st.type = 'stock_in' ORDER BY st.created_at DESC");
