...
$total_in = mysqli_fetch_assoc(mysqli_query($conn, "SELECT SUM(st.quantity) as total FROM stock_transactions st JOIN medicines m ON st.medicine_id = m.id WHERE st.type = 'stock_in' AND DATE_FORMAT(st.created_at, '%Y-%m') = '$month' $cat_filter"))['total'] ?? 0;
$total_out = mysqli_fetch_assoc(mysqli_query($conn, "SELECT SUM(st.quantity) as total FROM stock_transactions st JOIN medicines m ON st.medicine_id = m.id WHERE st.type = 'stock_out' AND DATE_FORMAT(st.created_at, '%Y-%m') = '$month' $cat_filter"))['total'] ?? 0;
$total_transactions = mysqli_fetch_assoc(mysqli_query($conn, "SELECT COUNT(*) as count FROM stock_transactions st JOIN medicines m ON st.medicine_id = m.id WHERE DATE_FORMAT(st.created_at, '%Y-%m') = '$month' $cat_filter"))['count'] ?? 0;
$most_used = mysqli_fetch_assoc(mysqli_query($conn, "SELECT m.name, SUM(st.quantity) as total FROM stock_transactions st JOIN medicines m ON st.medicine_id = m.id WHERE st.type = 'stock_out' AND DATE_FORMAT(st.created_at, '%Y-%m') = '$month' $cat_filter GROUP BY st.medicine_id ORDER BY total DESC LIMIT 1"));
$top_medicines = mysqli_query($conn, "SELECT m.name, SUM(st.quantity) as total FROM stock_transactions st JOIN medicines m ON st.medicine_id = m.id WHERE st.type = 'stock_out' AND DATE_FORMAT(st.created_at, '%Y-%m') = '$month' $cat_filter GROUP BY st.medicine_id ORDER BY total DESC LIMIT 6");
    ...
    <style>
        * { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        @media print {
            ...
            .main { margin-left: 0; }
            .logout-btn, .filter-bar, .menu-btn { display: none; }
...
<div class="main">
    ...
    <div class="stats-grid">
        <div class="stat-card"><div class="label">Total Stock In</div><div class="value"><?php echo number_format($total_in); ?></div><div class="sub">units received</div></div>
        <div class="stat-card"><div class="label">Total Stock Out</div><div class="value"><?php echo number_format($total_out); ?></div><div class="sub">units issued</div></div>
        <div class="stat-card"><div class="label">Total Transactions</div><div class="value"><?php echo $total_transactions; ?></div><div class="sub">this month</div></div>
        <div class="stat-card"><div class="label">Most Used Medicine</div><div class="value" style="font-size:16px"><?php echo $most_used ? $most_used['name'] : 'N/A'; ?></div><div class="sub"><?php echo $most_used ? number_format($most_used['total']).' units issued' : 'No data yet'; ?></div></div>
    </div>
        ...
        <?php if (count($top_data) > 0): ?>
            <?php foreach ($top_data as $item): ?>
                <div class="bar-row">
                    ...
                    <div class="bar-track"><div class="bar-fill" style="width:<?php echo ($item['total']/$max_usage)*100; ?>%"></div></div>
    ...
    <div class="section-title">Transaction History — <?php echo $category != '' ? $category.' — ' : ''; ?><?php echo date('F Y', strtotime($month.'-01')); ?></div>
    <div class="table-wrap">
        <table>
            <thead>
                <tr><th>#</th><th>Medicine</th><th>Type</th><th>Quantity</th><th>Done By</th><th>Date</th><th>Time</th></tr>
            </thead>
            <tbody>
            <?php
            $query = "SELECT st.*, m.name as medicine_name, u.name as user_name FROM stock_transactions st JOIN medicines m ON st.medicine_id = m.id JOIN users u ON st.user_id = u.id WHERE DATE_FORMAT(st.created_at, '%Y-%m') = '$month'";
            ...
            if (mysqli_num_rows($transactions) > 0) {
                ...
                while ($row = mysqli_fetch_assoc($transactions)) {
                    ...
                    echo "<tr><td>{$i}</td><td>{$row['medicine_name']}</td><td><span class='badge {$badge}'>{$label}</span></td><td>{$row['quantity']}</td><td>{$row['user_name']}</td><td>{$date}</td><td>{$time}</td></tr>";
                    $i++;
                }
            } else {
